  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (mode === 'single') {
        ...
        setCurrentStep(1)
        // Reversão parcial (09/09/2026): o texto literal citado entre aspas no brief
        // volta a ser renderizado pelo próprio modelo (sem overlay de canvas
        // pós-geração), sob regras rígidas — 1 linha, ~25 caracteres no máximo, sem
        // subtítulo, bem dentro da safe zone.
        const literalText = extractQuotedText(prompt)
        const textDirective = literalText
          ? ` MANDATORY TEXT TO RENDER: display exactly "${literalText}" as ONE single-line headline (about 25 characters or fewer, never wrapped to a second line), spelled exactly as given including every accent. No subtitle, no supporting line, no decorative badges or shapes around it. Keep it well inside the safe zone with generous empty margin to every edge.`
          : ' Do not render any text unless it is naturally part of the described scene.'
        const verticalPrompt = `Create a professional social media post. Content: ${prompt}. Vertical format.${textDirective} CRITICAL LAYOUT RULES: All text and visual elements must be strictly within the CENTER 55% of image width and CENTER 60% of image height. Use compact font sizes and tight line spacing. No text or elements near edges. No borders, frames or decorative containers. Background only in outer areas.`
        ...
        const slideContent = rawContent
          ...
          // Reversão parcial (09/09/2026): o headline do slide (slideContent) volta a
          // ser renderizado pelo próprio modelo — sem overlay de canvas pós-geração —
          // sob regras rígidas: 1 linha, ~25 caracteres no máximo, sem subtítulo/linha
          // de apoio, bem dentro da safe zone.
          const slidePromptText = i === 1
            ? `CAROUSEL COVER SLIDE 1 of ${slideCount}. MANDATORY TEXT TO DISPLAY: "${slideContent}". The headline shown in the image MUST be based strictly on this text — do not invent or replace it. Render it as ONE single line, about 25 characters or fewer, never wrapped. Design: bold full-bleed editorial image, ultra-bold single-line headline centered, strong visual hook that stops the scroll. Dark background, brand accent color on one key word, cinematic directional lighting. No subtitle, no supporting line, no decorative shapes around the text. NO robotic hands, NO AI cubes, NO generic tech imagery. Vertical 4:5 format. All text within the center 55% width and 60% height, kept well inside the safe zone with generous empty margin to every edge. No borders or frames.`
            : i === slideCount
            ? `CAROUSEL CLOSING SLIDE ${slideCount} of ${slideCount}. This is a CALL TO ACTION slide. The CTA phrase to display is: "${slideContent}". Show this as a bold visual CTA — large action words on ONE single line, about 25 characters or fewer, never wrapped, a highlighted button or underline on the key phrase. Do NOT write the word "CTA" — just display the action phrase visually. No subtitle, no supporting line. Dark background, brand accent color on the CTA element, cinematic mood. NO robotic hands, NO AI cubes. Vertical 4:5 format. All text within the center 55% width and 60% height, kept well inside the safe zone with generous empty margin to every edge. No borders or frames.`
            : `CAROUSEL SLIDE ${i} of ${slideCount}. THE HEADLINE FOR THIS SLIDE IS EXACTLY: "${slideContent}". Use these words as the main headline — do not substitute, do not invent a different headline, do not use the carousel topic as the headline. Render it as ONE single line, about 25 characters or fewer, never wrapped. Visual scene: ${scene}. Design: the headline IS the specific text provided above, no supporting line, no subtitle, no decorative shapes around the text. Dark background, brand accent color on one key word of the headline, cinematic lighting. NO robotic hands, NO AI cubes, NO generic tech imagery. Vertical 4:5 format. All text within the center 55% width and 60% height, kept well inside the safe zone with generous empty margin to every edge. No borders or frames.`
