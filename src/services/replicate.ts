...
export async function generateImage(imagePrompt: string, cost?: number, segment?: string, referenceImage?: string): Promise<string> {
  ...
  const res = await fetch('/api/generate-image-ai', {
    ...
    body: JSON.stringify({ prompt: imagePrompt, segment, ...(referenceImage ? { referenceImage } : {}) }),
  })
