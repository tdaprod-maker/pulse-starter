...
- CRITICAL CONTRAST: ensure high contrast between text and background — white text on dark areas, dark text on light areas, or a semi-transparent background strip when the underlying area doesn't have enough contrast on its own. Never render text directly over a busy or mid-tone area that lacks a clear contrast difference from the text color.
...
fake HDR.`
  ...
  // environment"). Safe-zone / posição / overlay de texto de carrossel /
  // letterboxing continuam de fora de propósito: eles são regras pra GERAR
  // texto novo e reposicionariam o texto já renderizado. A única exceção
  // (09/2026, causa raiz do texto ilegível no Premium) é CONTRASTE — trocar o
  // fundo pode deixar um texto que já era legível sem contraste contra a cena
  // nova, então buildTextTypographyRules() entra só pra dar a régua de
  // contraste, com prioridade explícita igual à do adjustPrompt: corrige a COR
  // do texto apenas se o novo fundo quebrar o contraste, nunca reposiciona,
  // redimensiona ou reescreve texto que já estava correto.
...
- Any text already rendered in the image — same wording, position, font, size
...
TEXT ALREADY IN THE IMAGE — its wording, position, size and font are correct: do NOT add,
remove, move, resize, rephrase, translate or restyle it. Its COLOR follows this priority:
- If the text's existing color already has high contrast against the new background behind
  it: leave the color exactly as it is.
- If the new background would leave the text with poor contrast (e.g. light text landing on
  a light area of the new scene, or dark text on a dark area): you MUST adjust ONLY the
  text's color (or add a subtle semi-transparent background strip behind it) so it stays
  clearly legible against the new scene. This is a color-only correction — wording,
  position, size and font stay exactly the same.
${buildTextTypographyRules()}
Do NOT add any logo or brand mark. Do NOT crop, pad, letterbox or change the aspect ratio.
Do NOT duplicate the person or generate extra people.
${photoIdentitySection}
...
of replacing it, moving or resizing text that only needed a color fix, rephrasing or
restyling text beyond the color-only contrast correction, plastic skin, symmetrical faces,
oversaturated colors, fake HDR, compositing halos, mismatched lighting between subject and
background, generic AI aesthetics, stock photo feeling.`
