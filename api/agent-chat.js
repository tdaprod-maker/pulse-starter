...
// Fallback: quando o modelo retorna mode:"carousel" sem o campo "slides" (deveria
// omiti-lo apenas quando o usuário não especificou títulos — se ele especificou e
// o modelo esqueceu de incluir, ou se o frontend precisa de algo para exibir antes
// da geração real via generate-carousel.js, isso garante que o campo sempre exista).
// Marcado com slidesAutoGenerated para que o frontend NÃO use esses títulos
// genéricos como override do conteúdo real gerado por generate-carousel.js.
function buildFallbackSlides(promptText, slideCount) {
  const topic = (promptText || '')
    .replace(/^SUJEITO:[\s\S]*?OBJETIVO:\s*/i, '')
    .replace(/["“”]/g, '')
    .trim()
    .slice(0, 80) || 'Tema do carrossel'
  const count = Math.max(1, Math.min(Number(slideCount) || 5, 10))

  const slides = []
  for (let i = 1; i <= count; i++) {
    if (i === 1) {
      slides.push({ title: topic })
    } else if (i === count) {
      slides.push({ title: 'Saiba mais', body: 'Confira mais detalhes e comece agora' })
    } else {
      slides.push({ title: `Ponto ${i - 1} sobre ${topic}` })
    }
  }
  return slides
}
...
engine válidos: "standard" ou "premium"`
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (parsed.ready && parsed.mode === 'carousel' && (!Array.isArray(parsed.slides) || parsed.slides.length === 0)) {
        console.log('[agent-chat] mode=carousel sem slides — aplicando fallback server-side')
        parsed.slides = buildFallbackSlides(parsed.prompt, parsed.slideCount)
        parsed.slidesAutoGenerated = true
      }
