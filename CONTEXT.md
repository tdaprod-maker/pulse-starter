...
| Ajuste pós-geração Premium | `runPremiumAdjust` no AgentChat: pedido em linguagem natural na imagem já gerada → confirma 4 pulses → `/api/generate-premium` usando a imagem como `visualReferences`, sem safe-zone/overlay/letterbox. `AgentChat.isRecomposeRequest()` escolhe `editMode: 'adjust'` (preservação total — "escurece o fundo") ou `editMode: 'recompose'` (preserva pessoa + texto, recria o cenário — "novo ambiente ao redor"; prompt enfático porque o gpt-image-2 tende a só reenquadrar). `AgentChat.isAddTextRequest()` detecta um terceiro caso — adicionar texto que não existia — e injeta as regras de safe-zone/tipografia nesse caminho (05/09/2026, ver Histórico de Bugs). Post único + por slide de carrossel Premium (alvo = slide visível no `CarouselViewer`). Viewers ressincronizam via `useEffect([slides])`. Pós-sucesso, `persistAdjustedPremium` sobrescreve o registro na Biblioteca (`premiumLibraryId`/`premiumCarouselLibraryId` vindos do `EditorPage`) — carrossel gerado no Editor não tem registro e não persiste |
...
7. Ajuste/recompose pós-geração via chat: qualquer pedido que não seja de logo → confirma 4 pulses → `runPremiumAdjust` reenvia a imagem gerada como `visualReferences` + `editMode: 'adjust'` ou `'recompose'` (decidido por `isRecomposeRequest`) → resultado substitui `premiumSlides` (post) ou o slide visível de `carouselSlides` (carrossel Premium) → `persistAdjustedPremium` sobrescreve o registro salvo na Biblioteca
...
28. **5 bugs reais encontrados em teste de uso (05/09/2026):**
    ...
    - **Premium: post sem texto mesmo pedindo, e ajuste pós-geração adicionava texto serifado/cortado** → (a) `PremiumPage.tsx` nunca passava `slideTitle` na geração de post único, então as regras mandatórias de texto (safe-zone, sans-serif, tamanho) só existiam pro carrossel; agora `extractQuotedText()` promove texto citado entre aspas no brief a `slideTitle`. (b) `adjustPrompt` em `api/generate-premium.js` sempre dizia "não mexa em texto nenhum", contradizendo pedidos de adicionar texto novo e deixando o gpt-image-2 sem nenhuma regra a seguir; `AgentChat.isAddTextRequest()` detecta esse caso e ativa uma variante do `adjustPrompt` com as regras de safe-zone/tipografia (extraídas para `buildTextTypographyRules()`, reaproveitada nos dois caminhos).
