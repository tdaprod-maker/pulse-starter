...
| Texto no Premium via overlay de canvas (não pelo modelo de imagem) | `src/services/textOverlay.ts` (`overlayTextOnImage`) — desde 05/09/2026, texto nunca é renderizado pelo gpt-image-2; ele gera a imagem sem texto e o headline/subtitle é desenhado depois via `<canvas>`, com safe-zone (3%/4.5%) garantida em código. Assinatura aceita `band` (topo/centro/base), `scale`, `color` e `font`. **Fontes (08/09):** `sans`→Sora, `serif`→Playfair, + `anton`/`archivo`/`bebas`/`oswald` (impact/condensadas); `FONT_WEIGHTS` por família (peso único → sem faux-bold). **Auto-fit vertical (08/09, "Fase 2"):** `firstBaselineY`/`renderedTop`/`renderedBottom` usam `TextMetrics.actualBoundingBoxAscent/Descent` real (não constante calibrada) — safe-zone garantida por medição; verificado em 36.288 casos simulados. **Texto estruturado (08/09, "Fase 3"):** `headline`/`subtitle` aceitam `string \| StyledLine[]` (`StyledLine = string \| TextRun[]`, `TextRun = { text; color? }`) — só cor varia por run, peso é do bloco; medição roda no texto achatado (neutra, validada). Controles no painel "Texto sobre a imagem": posição/tamanho/cor/fonte + **destaque (trecho + cor)** via `premiumCompose.buildHighlightedHeadline` (match só em fronteira de palavra). Usado em `PremiumPage.tsx` (geração) e via `composePremiumImage` no `EditorPage` |
